...
namespace Rope2d
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        {
            RefreshVisuals();
            if (generateStrechBridgeColliders) GenerateStretchBridgeColliders();
        }

        private void CopyInstanceData(Rope r)
        {
            isFadingOut = r.isFadingOut;
        }


        public void OnNodesCreated()
        {
            RefreshVisuals();
        }
        ...
        private void UpdateLineRenderer()
        {
            if (isLineRendering)
            {
                ...
                if (nodes != null)
                {
                    ...
                    {
                        var n0 = nodes[0];
                        Vector3 localHalfSide =
                            transform.InverseTransformVector(n0.transform.right *
                                                             (n0.nodeLength * 0.5f * nodeLineLengthFactor));
                        points = new Vector3[]
                            { n0.transform.localPosition - localHalfSide, n0.transform.localPosition + localHalfSide, };
                    }
        ...
        private void GenerateStretchBridgeColliders()
        {
            List<RopeNode> nodes = GetNodes();
            if (nodes != null)
            {
        ...
        public void SplitRopeAt(RopeNode node, bool before, bool isLastElement = false)
        {
            ...
            var leftRope = Instantiate(this, transform.parent);
            leftRope.transform.ClearChildrenOfType<SpriteRenderer>(true);
            foreach (var pair in nodes.Zip(leftRope.GetNodes(), (n1, n2) => (n1.rb, n2.rb)))
            {
            ...
            // var rightRope = Instantiate(this, transform.parent);
            // foreach (var pair in nodes.Zip(rightRope.GetNodes(), (n1, n2) => (n1.rb, n2.rb)))
            // {
            //     pair.Item2.velocity = pair.Item1.velocity;
            //     pair.Item2.angularVelocity = pair.Item1.angularVelocity;
            //     pair.Item1.constraints = RigidbodyConstraints2D.None;
            ...
            var fadeThis = nodes[0].GetFixedType() != FixEndType.Free;
            var fadeThat = isFadingOut || nodes[^1].GetFixedType() != FixEndType.Free;

            TrimRope(lastIndexToKeep, true);
            // leftRope.gameObject.SetActive(false);
            // gameObject.SetActive(false);
            leftRope.TrimRope(lastIndexToKeep, false);
            ...
            if (fadeThat) leftRope.FadeOut(1.5f, 4f);
        ...
        private void TrimRope(int index, bool leftSide)
        {
            ...
            if (nodes != null)
            {
                ...
                if (leftSide)
                {
                    ...
                    {
                        if (toDestroyNodes.Contains(nodes[i])) Destroy(nodes[i].gameObject);
                        else
                        {
                            var joints = nodes[i].GetComponents<Joint2D>();
                            foreach (var j in joints)
                            {
                ...
                DOTween.To(f =>
                        {
                            lr.colorGradient = new Gradient()
                            {
                                ...
                                alphaKeys = gradient0.alphaKeys.Select(k => new GradientAlphaKey(k.alpha * f, k.time))
                                    .ToArray(),
                            };
                        },
                        1,
