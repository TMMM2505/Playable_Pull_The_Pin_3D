namespace pancake.Rope2DEditor
{
    ...
    public class RopeMaker : MonoBehaviour
    {
        ...
        private IEnumerable<Vector3> AllLocalPoints => Enumerable.Repeat(end1, 1).Concat(midPoints).Append(end2);
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                if (isEnd1)
                    Gizmos.color = fixEnd1 == FixEndType.Fixed ? fixedColor :
                        fixEnd1 == FixEndType.Pivotable ? pvtColor : baseColor;
                else if (isEnd2)
                    Gizmos.color = fixEnd2 == FixEndType.Fixed ? fixedColor :
                        fixEnd2 == FixEndType.Pivotable ? pvtColor : baseColor;
        ...
        public void CreateRope()
        {
            ...
            transform.ClearChildrenOfType<SpriteRenderer>(true);
            ...
            while (pointIndex < smoothedPoints.Count)
            {
                ...
                node.transform.localRotation =
                    Quaternion.Euler(0, 0, Vector2.SignedAngle(Vector3.right, nextPoint - prevPoint));
            ...
            void SetupEnd(bool isFirst, RopeNode node, FixEndType fet)
            {
                ...
                if (anchorJoint)
                {
                    ...
                    float nodeJointOffsetX = node.GetComponent<CapsuleCollider2D>().size.x / 2f;
                    anchorJoint.autoConfigureConnectedAnchor = true;
                    anchorJoint.anchor = (isFirst ? Vector2.left : Vector2.right) * nodeJointOffsetX;

                    var pivot = ObjectUtils.InstantiatePrefab(pivotPrefab, transform);
                    pivot.transform.localPosition =
                        new Vector3(node.transform.localPosition.x + (isFirst ? -1 : 1) * nodeJointOffsetX * node.transform.localScale.x, 0, 0);
                }
            }

            if (nodes.Count > 0) SetupEnd(true, nodes[0], fixEnd1);
            if (nodes.Count > 1) SetupEnd(false, nodes[^1], fixEnd2);
    ...
    public class RopeMakerEditor : Editor
    {
        ...
        {
            TryCreateRope(target as RopeMaker, false);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorUtilEx.MiniBoxedSection(
                    string.Format("<color={0}><b>WARNING</b></color>", ColorUtils.RichTextTag(Color.red)),
                ...
                EditorUtilEx.MiniBoxedSection(
                    ...
                    () =>
                    {
                        EditorGUILayout.LabelField(
                            "To edit this RopeMaker, you need to disconnect/unpack prefab. Or press start editing below.");
                        if (EditorUtilEx.MyButton("Start editing", Color.yellow))
                        {
                            PrefabUtility.UnpackPrefabInstance(ropeMaker.gameObject, PrefabUnpackMode.OutermostRoot,
            ...
            EditorUtilEx.MiniBoxedSection("Editing",
                () =>
                {
                    ...
                    if (newScale != ropeMaker.scale || newSpacing != ropeMaker.spacing ||
                        newSmoothness != ropeMaker.smoothness)
                    {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        var rectBase = GUILayoutUtility.GetRect(GUIContent.none, EditorStyles.helpBox,
                            GUILayout.Width(200));
