...
namespace Dreamteck.Splines.IO
{
    public class SVG : SplineParser
    {
        ...
        Vector2 MapPoint([Bridge.Ref] Vector3 original, Axis ax)
        {
            switch (ax)
            {
        ...
        public List<SplineComputer> CreateSplineComputers([Bridge.Ref] Vector3 position, [Bridge.Ref] Quaternion rotation, Element elements = Element.All)
        {
        ...
        private void CalculateEllipseParams([Bridge.Ref] Vector2 p0, [Bridge.Ref] Vector2 p1, float phi, float rx, float ry, bool fa, bool fs, out Vector2 c, out float theta1, out float sweepTheta, out float adjustedRx, out float adjustedRy)
        {
        ...
        private float VectorAngle([Bridge.Ref] Vector2 u, [Bridge.Ref] Vector2 v)
        {
