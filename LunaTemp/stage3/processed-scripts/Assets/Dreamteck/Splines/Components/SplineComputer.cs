namespace Dreamteck.Splines
{
    ...
    public partial class SplineComputer : MonoBehaviour
    {
        ...
        public void SetPointPosition(int index, [Bridge.Ref] Vector3 pos, Space setSpace = Space.World)
        {
            if (index < 0) return;
            ResampleTransformIfNeeded();
        ...
        /// <param name="setSpace"></param>
        public void SetPointTangents(int index, [Bridge.Ref] Vector3 tan1, [Bridge.Ref] Vector3 tan2, Space setSpace = Space.World)
        {
            if (index < 0) return;
            ResampleTransformIfNeeded();
            if (index >= _spline.points.Length)
        ...
        /// <param name="setSpace"></param>
        public void SetPointNormal(int index, [Bridge.Ref] Vector3 nrm, Space setSpace = Space.World)
        {
            if (index < 0) return;
            ResampleTransformIfNeeded();
            if (index >= _spline.points.Length)
        ...
        public void SetPointColor(int index, [Bridge.Ref] Color color)
        {
            if (index < 0) return;
            if (index >= _spline.points.Length)
            {
        ...
        public void Project(ref SplineSample result, [Bridge.Ref] Vector3 position, double from = 0.0, double to = 1.0, EvaluateMode mode = EvaluateMode.Cached, int subdivisions = 4)
        {
        ...
        /// <returns></returns>
        public void Project([Bridge.Ref] Vector3 worldPoint, ref SplineSample result, double from = 0.0, double to = 1.0, EvaluateMode mode = EvaluateMode.Cached, int subdivisions = 4)
        {
            if (mode == EvaluateMode.Calculate)
            {
        ...
        public SplineSample Project([Bridge.Ref] Vector3 worldPoint, double from = 0.0, double to = 1.0)
        {
            SplineSample result = new SplineSample();
        ...
        /// <param name="address">Node address of junctions</param>
        /// <returns></returns>
        public bool Raycast(out RaycastHit hit, out double hitPercent, [Bridge.Ref] LayerMask layerMask, double resolution = 1.0, double from = 0.0, double to = 1.0 , QueryTriggerInteraction hitTriggers = QueryTriggerInteraction.UseGlobal)
        {
        ...
        /// <param name="address">Node address of junctions</param>
        /// <returns></returns>
        public bool RaycastAll(out RaycastHit[] hits, out double[] hitPercents, [Bridge.Ref] LayerMask layerMask, double resolution = 1.0, double from = 0.0, double to = 1.0, QueryTriggerInteraction hitTriggers = QueryTriggerInteraction.UseGlobal)
        {
            resolution = DMath.Clamp01(resolution);
            Spline.FormatFromTo(ref from, ref to, false);
            double percent = from;
        ...
        public SplineTrigger AddTrigger(int triggerGroup, double position, SplineTrigger.Type type, string name, [Bridge.Ref] Color color)
        {
        ...
        private void SetNodeForPoint(int index, [Bridge.Ref] SplinePoint worldPoint)
        {
            for (int i = 0; i < _nodes.Length; i++)
            {
        ...
        public Vector3 TransformPoint([Bridge.Ref] Vector3 point)
        {
#if UNITY_EDITOR
        ...
        public Vector3 InverseTransformPoint([Bridge.Ref] Vector3 point)
        {
#if UNITY_EDITOR
        ...
        public Vector3 TransformDirection([Bridge.Ref] Vector3 direction)
        {
#if UNITY_EDITOR
        ...
        public Vector3 InverseTransformDirection([Bridge.Ref] Vector3 direction)
        {
#if UNITY_EDITOR
