...
namespace Dreamteck.Splines {
    ...
    public class SplineUser : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public double TravelWithOffset(double start, float distance, Spline.Direction direction, [Bridge.Ref] Vector3 offset, out float moved)
        {
            moved = 0f;
            if (direction == Spline.Direction.Forward && start >= 1.0)
            {
        ...
        public virtual void Project([Bridge.Ref] Vector3 position, ref SplineSample result, double from = 0.0, double to = 1.0)
        {
        ...
        public float CalculateLengthWithOffset([Bridge.Ref] Vector3 offset, double from = 0.0, double to = 1.0)
        {
        ...
        /// <returns></returns>
        protected static Vector3 TransformOffset([Bridge.Ref] SplineSample sample, [Bridge.Ref] Vector3 localOffset)
        {
