namespace pancake.Rope2DExtension
{
    ...
    public static class ObjectUtils
    {
            ...
            where T : Component
        {
            if (getBounds == null)
                getBounds = (
                    t) => (t as Collider)?.bounds ?? (t as Collider2D)?.bounds ?? (t as Renderer)?.bounds ?? default(UnityEngine.Bounds);
            var comps = go.GetComponentsInChildren<T>(includeInactive);

            Bounds b0 = default(UnityEngine.Bounds);
            bool found = false;
